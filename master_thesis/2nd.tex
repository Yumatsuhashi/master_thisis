...
\section{はじめに}
本章では5G NR物理層におけるチャネル予測の基礎事項を整理する．
まず5G NRの無線伝送処理として，リソースグリッド，参照信号，チャネル推定，プリコーディング，ビームフォーミングを概説する．
続いて，チャネル予測の動作原理と関連研究を整理し，本論文が取り組む課題を明確にする．



\section{5G NRの物理層における無線伝送処理}
本節では，5G NRの物理層で用いられる無線伝送処理を概説する\cite{HattoriFujioka2018_5GTextbook}．
5G NRではOFDMによる広帯域伝送と複数アンテナによるMIMO伝送が採用されており，送信アンテナ数を$N_t$，受信アンテナ数を$N_r$，サブキャリア添字を$f$，時間添字を$t$とすると，チャネル行列$\mathbf{H}(f,t)\in \mathbb{C}^{N_r\times N_t}$により送受信信号の対応関係が表される．
以降では，リソースグリッド上の信号配置，参照信号に基づくチャネル推定，プリコーディングとビームフォーミングの順に述べる．
...
OFDMでは，時間領域の畳み込みが周波数領域でサブキャリアごとの乗算として表される．
実際の通信では，周波数軸に加えて時間軸にも無線資源が割り当てられる．
時間と周波数で張られる二次元平面をリソースグリッドと呼ぶ．
リソースグリッド上の最小単位をリソースエレメント(RE)と呼ぶ．REは1本のサブキャリアと1つのOFDMシンボルで定義される．
通信システムはリソースグリッド上に，ユーザデータ，制御信号，チャネル推定に用いる参照信号(RS)を配置して伝送する．受信信号は$\mathbf{y}(f,t)$と表され，データまたは参照信号が割り当てられたREで観測される．

\begin{figure}[H]
  \centering
  \includegraphics[width=100mm]{./img/2nd/2_1_pdf15.pdf}
  \caption{5G NRにおけるフレーム構造の例（SCSごとのスロット数）}
  \label{fig:2_1}
\end{figure}
...
また，OFDMではマルチパス遅延によるシンボル間干渉を抑えるため，サイクリックプレフィクス(CP)を付加する．5G NRではCP長に応じてnormal CPとextended CPを規定しており，normal CPでは1スロットは14個のOFDMシンボル，extended CPでは12個のOFDMシンボルで構成される\cite{3gpp_38211}．
...
\paragraph{周波数方向の構造}
周波数方向では，サブキャリアを12本束ねた単位をリソースブロック(RB)と呼ぶ．
RBにより，スケジューリングや参照信号配置をブロック単位で表せる．
広帯域伝送ではマルチパス遅延により周波数選択性が生じるため，サブキャリアごとにチャネル行列$\mathbf{H}(f,t)$が定義される．

\subsubsection{TDD方式と参照信号}
リソースグリッド上の信号配置と送受信のタイミングは，複信方式によって決定される．
特に5G NR以降の移動体通信システムでは高い周波数帯の利用が進み，上りリンク(UL)と下りリンク(DL)で同一の周波数帯域を用いて時間方向に通信方向を切り替える方式が広く用いられる．この方式を時分割複信(TDD)と呼ぶ．

TDDではチャネル状態情報(CSI)を得るために参照信号を用いる．代表例として，DLでは基地局が送信し端末が受信するCSI-RS，ULでは端末が送信し基地局が受信するSRSが挙げられる．
従来の周波数分割複信(FDD)では，基地局がCSI-RSを送信し，端末が推定したCSIを基地局へフィードバックする運用が主であった．
...
\end{equation}
と表される．$\mathbf{X}(f,t)$は既知である．$\mathbf{N}(f,t)$は受信雑音であり，熱雑音に起因する加法性白色ガウス雑音(AWGN)としてモデル化する場合が多い．
...
最小二乗(LS)推定は，観測とモデルの差である残差が最も小さくなる$\mathbf{H}$を選ぶ推定である．
...
そこで事前統計を利用し，推定誤差の平均二乗誤差を小さくすることを考える線形最小平均二乗誤差(LMMSE)推定を導入する．
...
以下では，$\mathbf{P}$の代表的な設計を示す．重みは推定チャネル$\hat{\mathbf{H}}$に基づいて計算されるが，表記を簡単のため$\mathbf{H}$と書く．
...
\label{eq:prec_ls}
\end{equation}
となる．この解はゼロフォーシング(ZF)プリコーディングとして知られる．ZFは干渉を抑圧できる一方，$\mathbf{H}\mathbf{H}^{\mathrm{H}}$の条件数が大きい場合に雑音が増幅しやすい．
...
複数の送受信アンテナを用いる多入力多出力(MIMO)伝送がもたらす利得は，空間多重，空間多様性，ビームフォーミングに整理できる．空間多重は複数ストリームを同時に送信してスループットを向上させる．空間多様性は同一情報を冗長に分散してフェージングの影響を低減する．ビームフォーミングは所望ユーザ方向の有効利得を高め，他ユーザ方向への漏洩を抑える．
...
代表例として，単一ユーザで雑音が支配的な場合には最大比送信(MRT)が用いられ，
\begin{equation}
...
単一ユーザの$N_s>1$ストリーム送信では，$\mathbf{w}$を1本選ぶ代わりに，$\mathbf{P}$の列ベクトルとして複数本のビームを設計する．この単一ユーザMIMO(SU-MIMO)では，代表的な理論モデルとして特異値分解(SVD)を用いる．
\begin{equation}
...
\section{チャネル予測の動作原理}
本節では，CSIを将来時刻へ外挿するCSI予測を扱う\cite{adeogun2025_channel_prediction_survey, jiang2025_ai_csi_prediction_5g}．
まず，5Gの進展に伴ってCSIの取得と更新が抱える課題を整理し，CSI予測が必要となる背景を述べる．
次に，CSI予測の代表的な枠組みを概観した上で，時系列CSI予測に焦点を当てて関連手法を整理する．


5Gでは高周波数帯の利用や高密度化が進み，基地局は多数のアンテナ素子を用いた大規模MIMO(mMIMO)により，空間多重とビーム運用で容量を確保する．
...
端末(UE)側でCSIを低次元表現へ圧縮して送信し，基地局(BS)側で復元する枠組みが提案されている\cite{li2019_dl_mimo_csi_feedback}．
...
環境適応に関する研究は，転移学習，メタ学習，オンライン学習，継続学習など複数のアプローチに分類できる．
転移学習に基づくアプローチとして，Yangらは環境変化による性能劣化を転移学習問題として定式化し，既存環境で訓練したモデルを新環境で微調整することで適応力を向上させた\cite{yang2020deep}．

シミュレーションと実環境間のギャップに着目した研究も進んでおり，Hoangらはチャネルモデルで事前学習したモデルを実データで微調整し，限られた実測CSIでも高い予測性能を達成できることを示した\cite{hoang2025map}．

また，統計的チャネルモデルと地図ベースチャネルモデル間のドメインギャップをドメイン適応で埋め，シミュレーションから実環境への汎化性能を向上させた研究もある\cite{hoang2025domain}．

転移学習に関する研究として

メタ学習に基づくアプローチでは，少量データでの高速適応を狙った初期化の学習が検討されている．
...
事前学習済みモデルの転用も環境変化に対する汎化を狙う方向性であり，Liuらは大規模言語モデルをCSI予測へ適用し，少量データ学習と未学習環境での性能を評価した\cite{liu2024_llm4cp}．

オンライン学習に基づくアプローチでは，Xuらがパイロット信号のみで逐次学習するモデルにより，オフライン学習モデルの環境不一致を上回る性能を達成できることを報告している\cite{xu2024learning}．
運用中の分布変化に対しては，継続学習により逐次追従する枠組みも提案されている\cite{mohsin2025_continual_learning_channel_prediction}．
特にハンドオーバー時の環境変化に対しては，Mohsinらが継続学習問題として定式化し，リプレイ，正則化，蒸留を組み合わせて忘却を抑制しつつ新環境へ適応する手法を提案している\cite{mohsin2025continual}．

チャネル予測モデルの重みを更新せずに疎なパイロット信号を補間する試みも検討されている．
Chenらは，空間的および周波数的に部分的に推定したCSIをCDNetに入力し，パイロットが送信されないスロットの値も継続的に入力して逐次予測する手法を提案した\cite{chen2025_channel_deduction}．
Liらは，予測時刻でパイロットが欠如する条件に対し，VAEによる補間を用いて逐次的な予測を実現している\cite{li2024_incomplete_history_prediction}．
Tekgulらは，入力の欠如をそのまま欠落として扱い逐次的な予測を実現している\cite{tekgul2022_incomplete_history}．
...
他分野では観測や正解データが継続的に得られるため，オンライン学習\cite{cover1991_universal_portfolios}\cite{kalnay2003_atmospheric_modeling,evensen2003_enkf}により逐次更新する枠組みが一般的である．
...
本論文では，第2および第3の方針が想定する状況を対象として，予測を継続したまま欠損する正解ラベルを扱い，モデルを更新する問題に取り組む．

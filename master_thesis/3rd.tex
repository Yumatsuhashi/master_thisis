...
\label{chap:3rd}
\section{はじめに}
前章では，時系列CSI予測の運用において，参照信号削減に伴い予測スロットで正解ラベルが欠損する問題を述べた．
予測を継続しながらモデルを更新するには，欠損する正解ラベルを何らかの方法で補う必要がある．
本章では，この欠損を補う手段として時系列補間に着目し，チャネル予測モデルの重み更新に適した補間手法を選定する．

補間の基本方針は，欠損時刻の前後に存在する推定値から補間値を算出し，これを更新用サンプルの正解ラベルとして用いることである．
補間対象時刻より後に得られる推定値も活用するため，一方向の外挿と比較して補間誤差を抑えられる．
本章では，線形補間，スプライン補間，多項式近似，ニューラルネットワーク補間の4種類を比較対象とする．

評価にはレイトレーシングにより生成したCSI系列を用いる．
東京都内の複数地点の地形データを三次元モデル化し，歩行軌道に沿ったCSI系列を生成する．
パイロット信号の送信周期を変化させた複数の観測パターンを設定し，欠損率が25\%から50\%に変化した場合の補間精度を評価する．

本章の構成は以下のとおりである．
第\ref{sec:interpolation_methods}節では，比較対象とする4種類の補間手法を説明する．
第3.3節では，シミュレーションに用いる地形データとレイトレーシングの設定を述べる．
第3.4節および第3.5節では，各補間手法の精度を評価し，チャネル予測モデルの重み更新に適した手法を選定する．
...
近傍点数の比較では，参照信号削減に伴う欠測の周期を4とし，欠測時刻に対応する時刻差を近傍候補から除外する．
具体的には，\(\mathbb{Z}\)を整数集合として，周期を$N$とすると候補集合を
\begin{equation}
...
\paragraph{MLP補間器}
基本設定の補間器は全結合層からなるMLPとして実装する．context\_size\(=3\)のとき，6F次元の入力を2048次元へ変換しReLUで活性化する第1層，2048次元を1024次元へ変換しReLUで活性化する第2層，1024次元をF次元へ変換する出力層の3層構造とする．

\paragraph{RMS正規化}
チャネル全体のスケール変動に対する学習の安定化を目的として，入力のRMSによる正規化を施す．入力\(\bm{x}_t\)のRMSから正規化係数\(\alpha_t\)を算出し，正規化後の入力\(\bar{\bm{x}}_t=\bm{x}_t/\alpha_t\)を補間器へ与える．補間器の出力は元スケールへ戻し，\(\widetilde{\bm{h}}_t=\alpha_t\,g_{\phi}(\bar{\bm{x}}_t)\)から\(\widetilde{\bm{H}}_t\)を得る．この正規化は学習と推論の双方で共通して適用する．
...
PRMは複雑な障害物環境における経路計画アルゴリズムである．本手法はロードマップ構築フェーズと経路探索フェーズの二段階で構成され，事前に構築したグラフ構造を用いて効率的な経路探索を実現する．

ロードマップ構築フェーズでは，移動可能な経路網をグラフとして表現する．最初に，対象領域内にランダムにノードを配置する．配置したノードのうち建物内部に位置するものは移動経路として不適切であるため，各ノードから鉛直上方にレイを照射して建物との衝突判定を行い，屋内と判定されたノードを除外する．屋外ノードのみを抽出した後，KDTreeを用いたK近傍探索により各ノードの接続候補を選定する．接続候補間では見通し線チェックを実施し，二点間にレイを照射して障害物との衝突がなければエッジを作成する．この手順により，建物を貫通せずに歩行可能な経路のみで構成される無向グラフが得られる．

経路探索フェーズでは，構築したロードマップ上でA*アルゴリズムを適用し，出発点から目的地までの最短経路を探索する．A*アルゴリズムは各ノード$n$に対して評価関数$f(n) = g(n) + h(n)$を計算し，$f(n)$が最小となるノードを優先的に展開する．$g(n)$は出発点からノード$n$までに要した実コストを表し，$h(n)$はノード$n$から目的地までのユークリッド距離をヒューリスティックとして用いる．このヒューリスティックは真のコストを過大評価しないため，A*アルゴリズムは最適経路を保証する．得られた経路はノード列の折れ線となるため，指定した歩行速度とサンプリング間隔に基づいて等時間間隔でリサンプリングし，時系列の座標列として出力する．

PRMは格子状探索と比較して少ないノード数で広域をカバーでき，計算効率が高い．一度構築したロードマップは複数の出発点と目的地の組み合わせに対して再利用できるため，多数の経路を生成する場合に有効である．不規則に建物が配置された都市環境においても柔軟に適用できる点も利点として挙げられる．
...
\begin{align}
  ...
  \text{BS縦方向素子間隔：}\texttt{bs\_antenna\_vertical\_spacing} &= 0.5 \\
  \text{BS横方向素子間隔：}\texttt{bs\_antenna\_horizontal\_spacing} &= 0.5 \\
  \text{端末アンテナ列数：}\texttt{user\_antenna\_rows} &= 2 \\
  \text{端末アンテナ行数：}\texttt{user\_antenna\_cols} &= 1 \\
  \text{UE縦方向素子間隔：}\texttt{ue\_antenna\_vertical\_spacing} &= 0.5 \\
  \text{UE横方向素子間隔：}\texttt{ue\_antenna\_horizontal\_spacing} &= 0.5 \\
  ...
  \text{OFDM変換正規化：}\texttt{cir\_to\_ofdm\_normalize} &= \texttt{false}
\end{align}
}
...
\end{equation}
となる．SRSは周波数方向に一定間隔で配置されるとし，SRSが配置された周波数点においてCSIを算出する．
...
補間精度の評価には，第\ref{subsec:terrain_raytracing}節で述べた錦糸町および八重洲の2種類の地形データに対してレイトレーシングにより生成したCSI系列を用いる．
補間手法としては，第\ref{sec:interpolation_methods}節で述べた線形補間，スプライン補間，多項式近似，ニューラルネットワーク補間を比較対象とする．
ニューラルネットワーク補間器の学習には，評価対象とは異なる地形データから生成したCSI系列を用いる．
錦糸町駅前のCSI系列を補間するモデルは，新宿，池袋，渋谷の3地点から生成したCSI系列で学習する．
八重洲のCSI系列を補間するモデルは，八重洲から生成したCSI系列を学習データとして用いる．
...
パイロット信号の送信周期が変化した場合における補間精度を評価するため，推定値と予測値の配置を変化させた複数の疎化パターンを設定する．
疎化パターンは，推定値$\bm{H}$が得られるスロットと予測値$\widehat{\bm{H}}$で代替するスロットの周期的な配置として定義する．
周期$N$で1個の予測スロットを含むパターンを周期$N$出力1パターンと呼ぶ．
本章では以下の3種類を用いる．
\begin{align}
\text{周期4出力1パターン} &: \; \bm{H},\bm{H},\bm{H},\widehat{\bm{H}} \; \text{を周期4で繰り返し（削減率25\%）} \\
\text{周期3出力1パターン} &: \; \bm{H},\bm{H},\widehat{\bm{H}} \; \text{を周期3で繰り返し（削減率33\%）} \\
\text{周期2出力1パターン} &: \; \bm{H},\widehat{\bm{H}} \; \text{を周期2で繰り返し（削減率50\%）}
\end{align}
周期4出力1パターンでは4スロットに1回，周期3出力1パターンでは3スロットに1回，周期2出力1パターンでは2スロットに1回の頻度で補間が必要となる．
疎化パターンに応じて削減率が25\%から50\%まで変化するため，各パターンにおいて有効な補間手法を特定する．
各パターンにおいて，周期$N$スロットのうち最後の1スロットが補間対象となり，残りの$N-1$スロットでは推定値が得られる．

評価指標には正規化平均二乗誤差NMSEを用いる．
補間対象時刻$t$における補間値$\widetilde{\bm{H}}_t$と真値$\bm{H}_t$の誤差を
\begin{equation}
  \mathrm{NMSE}(t)
...
評価は周期4出力1パターンを用いて実施し，各補間手法のNMSEを比較する．
...
\begin{table}[H]
  \centering
  \caption{錦糸町データセットにおける周期4出力1パターンの補間精度}
  ...
  \begin{tabular}{llc}
    ...
    \hline
    線形補間 & -- & 0.375 \\
    スプライン補間 & -- & \textbf{0.00610} \\
    多項式近似 & 2次，片側3点ずつ & 0.0517 \\
    多項式近似 & 3次，片側3点ずつ & 0.0517 \\
    多項式近似 & 4次，片側3点ずつ & \textbf{0.00590} \\
    多項式近似 & 5次，片側3点ずつ & \textbf{0.00590} \\
    多項式近似 & 6次，片側3点ずつ & 0.143 \\
    多項式近似 & 4次，片側6点ずつ & 0.229 \\
    多項式近似 & 4次，片側9点ずつ & 0.258 \\
    ニューラルネットワーク補間 & $c=1$ & 0.385 \\
    ニューラルネットワーク補間 & $c=2$ & 0.511 \\
    ニューラルネットワーク補間 & $c=3$ & 0.474 \\
    \hline
  \end{tabular}
\end{table}
...
\subsubsection{八重洲（LoS）データセットにおける補間精度}
八重洲（LoS）の地形データから生成した101ファイルのCSI系列に対する補間精度評価を表\ref{tab:marunouchi_sssp_results}に示す．

\begin{table}[H]
  \centering
  \caption{八重洲（LoS）データセットにおける周期4出力1パターンの補間精度}
  ...
  \begin{tabular}{llc}
    ...
    多項式近似 & 6次，片側3点ずつ & 0.152 \\
    多項式近似 & 4次，片側6点ずつ & 0.152 \\
    多項式近似 & 4次，片側9点ずつ & 0.157 \\
    ニューラルネットワーク補間 & $c=1$ & 0.145 \\
    ニューラルネットワーク補間 & $c=2$ & 0.131 \\
    ニューラルネットワーク補間 & $c=3$ & 0.491 \\
    \hline
  \end{tabular}
\end{table}

八重洲（NLoS）データセットに対する周期4出力1パターンの補間精度評価を表\ref{tab:marunouchi_nlos_sssp_results}に示す．
八重洲（NLoS）データセットの評価では，線形補間およびニューラルネットワーク補間を比較対象から除外した．
錦糸町および八重洲（LoS）データセットの評価において，線形補間は補間対象の前後1点のみを用いる単純な手法であり，NMSEが0.16から0.37と低精度であることが確認された．
ニューラルネットワーク補間も学習データと評価データの地点差に起因するドメインシフトにより解析的手法を下回る精度にとどまった．
これらの結果から，八重洲（NLoS）データセットにおいても同様の傾向が予想されるため，評価対象から除外した．

\begin{table}[H]
  \centering
  \caption{八重洲（NLoS）データセットにおける周期4出力1パターンの補間精度}
  \label{tab:marunouchi_nlos_sssp_results}
  \begin{tabular}{llc}
    \hline
    手法 & 設定 & NMSE \\
    \hline
    スプライン補間 & -- & \textbf{0.0574} \\
    多項式近似 & 2次，片側3点ずつ & 0.0824 \\
    多項式近似 & 3次，片側3点ずつ & 0.0824 \\
    多項式近似 & 4次，片側3点ずつ & 0.0580 \\
    多項式近似 & 5次，片側3点ずつ & 0.0580 \\
    多項式近似 & 6次，片側3点ずつ & 0.174 \\
    多項式近似 & 4次，片側6点ずつ & 0.214 \\
    多項式近似 & 4次，片側9点ずつ & 0.225 \\
    \hline
  \end{tabular}
\end{table}

八重洲（LoS）データセットにおいても，スプライン補間と4次または5次の多項式近似が最良の精度を示し，NMSEは0.04から0.05程度であった．
ニューラルネットワーク補間の精度は錦糸町データセットと比較して向上している．これは，八重洲（LoS）データセットでは学習と評価が同一地点のCSI系列で構成されているためである．
...
前節では周期4出力1パターンを用いて評価したが，本節では周期3出力1パターンおよび周期2出力1パターンを追加し，削減率が33\%および50\%に増加した場合の補間精度を比較する．

\subsection{結果}

\subsubsection{錦糸町データセットにおける補間精度}
錦糸町データセットに対する周期3出力1パターンおよび周期2出力1パターンの補間精度評価を表\ref{tab:kinshicho_ssp_sp_results}に示す．

\begin{table}[H]
  \centering
  \caption{錦糸町データセットにおける周期3出力1および周期2出力1パターンの補間精度}
  ...
  \begin{tabular}{llrr}
    ...
    \hline
    線形補間 & -- & 0.275 & 0.287 \\
    スプライン補間 & -- & \textbf{0.00620} & \textbf{0.0130} \\
    多項式近似 & 2次，片側3点ずつ & 0.117 & 0.176 \\
    多項式近似 & 3次，片側3点ずつ & 0.117 & 0.176 \\
    多項式近似 & 4次，片側3点ずつ & \textbf{0.00690} & \textbf{0.0162} \\
    多項式近似 & 5次，片側3点ずつ & \textbf{0.00690} & \textbf{0.0162} \\
    多項式近似 & 6次，片側3点ずつ & 0.159 & 0.218 \\
    多項式近似 & 4次，片側6点ずつ & 0.163 & 0.224 \\
    多項式近似 & 4次，片側9点ずつ & 0.280 & 0.313 \\
    ニューラルネットワーク補間 & $c=1$ & 0.312 & 0.378 \\
    ニューラルネットワーク補間 & $c=2$ & 0.427 & 0.397 \\
    ニューラルネットワーク補間 & $c=3$ & 0.576 & 0.596 \\
    \hline
  \end{tabular}
\end{table}

周期3出力1パターンにおいても，スプライン補間と4次多項式近似が最良の精度を示し，NMSEは0.006から0.007程度であった．
周期2出力1パターンでは削減率が50\%に達するが，スプライン補間と4次多項式近似はNMSE 0.013から0.016程度を維持した．
ニューラルネットワーク補間は全パターンで0.3以上のNMSEとなり，学習データと評価データの地点差による汎化性能の低さが確認された．

以降の周期3・2出力パターンおよび周期4・3出力パターンの評価では，線形補間およびニューラルネットワーク補間を比較対象から除外した．
周期4出力1，周期3出力1，周期2出力1パターンの評価において，線形補間はNMSEが0.16から0.37と低精度であり，補間対象の前後1点のみを用いる単純な手法では時間変動を十分に捉えられないことが確認された．
...
\noindent
錦糸町データセットに対する周期3・2出力パターンおよび周期4・3出力パターンの補間精度評価をそれぞれ表\ref{tab:kinshicho_spp_results}および表\ref{tab:kinshicho_sppp_results}に示す．
本実験では補間位置が複数存在するため，補間位置ごとのNMSE（pos）と，それらを統合したOverall NMSEを算出した．

\begin{table}[H]
  \centering
  \caption{錦糸町データセットにおける周期3・2出力パターンの補間精度}
  \label{tab:kinshicho_spp_results}
  \begin{tabular}{llrrr}
    \hline
    手法 & 設定 & Overall NMSE & pos0 & pos1 \\
    \hline
    多項式近似 & 5次，片側3点ずつ & \textbf{0.129} & 0.128 & 0.129 \\
    スプライン補間 & 片側3点ずつ & 0.131 & 0.131 & 0.132 \\
    スプライン補間 & 片側6点ずつ & 0.136 & 0.136 & 0.137 \\
    スプライン補間 & 片側9点ずつ & 0.136 & 0.136 & 0.137 \\
    多項式近似 & 4次，片側3点ずつ & 0.169 & 0.169 & 0.169 \\
    多項式近似 & 3次，片側3点ずつ & 0.245 & 0.246 & 0.245 \\
    多項式近似 & 2次，片側3点ずつ & 0.249 & 0.250 & 0.249 \\
    多項式近似 & 6次，片側6点ずつ & 0.265 & 0.265 & 0.265 \\
    多項式近似 & 5次，片側6点ずつ & 0.309 & 0.310 & 0.309 \\
    多項式近似 & 4次，片側6点ずつ & 0.312 & 0.312 & 0.311 \\
    多項式近似 & 6次，片側9点ずつ & 0.337 & 0.337 & 0.336 \\
    多項式近似 & 5次，片側9点ずつ & 0.371 & 0.371 & 0.371 \\
    多項式近似 & 4次，片側9点ずつ & 0.371 & 0.372 & 0.370 \\
    多項式近似 & 6次，片側3点ずつ & 0.419 & 0.419 & 0.419 \\
    \hline
  \end{tabular}
\end{table}
...
\begin{table}[H]
  \centering
  \caption{錦糸町データセットにおける周期4・3出力パターンの補間精度}
  \label{tab:kinshicho_sppp_results}
  \begin{tabular}{llrrrr}
    \hline
    手法 & 設定 & Overall NMSE & pos0 & pos1 & pos2 \\
    \hline
    多項式近似 & 5次，片側3点ずつ & \textbf{0.276} & 0.209 & 0.409 & 0.209 \\
    スプライン補間 & 片側3点ずつ & 0.282 & 0.213 & 0.421 & 0.213 \\
    スプライン補間 & 片側6点ずつ & 0.283 & 0.213 & 0.422 & 0.214 \\
    スプライン補間 & 片側9点ずつ & 0.283 & 0.213 & 0.422 & 0.214 \\
    多項式近似 & 4次，片側3点ずつ & 0.336 & 0.299 & 0.409 & 0.299 \\
    多項式近似 & 3次，片側3点ずつ & 0.345 & 0.324 & 0.390 & 0.323 \\
    多項式近似 & 6次，片側6点ずつ & 0.352 & 0.338 & 0.380 & 0.337 \\
    多項式近似 & 2次，片側3点ずつ & 0.365 & 0.354 & 0.390 & 0.353 \\
    多項式近似 & 5次，片側6点ずつ & 0.365 & 0.360 & 0.376 & 0.360 \\
    多項式近似 & 4次，片側6点ずつ & 0.371 & 0.369 & 0.376 & 0.368 \\
    多項式近似 & 6次，片側9点ずつ & 0.379 & 0.377 & 0.383 & 0.376 \\
    多項式近似 & 5次，片側9点ずつ & 0.403 & 0.402 & 0.405 & 0.403 \\
    多項式近似 & 4次，片側9点ずつ & 0.404 & 0.404 & 0.405 & 0.404 \\
    多項式近似 & 6次，片側3点ずつ & 0.505 & 0.399 & 0.727 & 0.388 \\
    \hline
  \end{tabular}
\end{table}
...
\subsubsection{八重洲（LoS）データセットにおける補間精度}
八重洲（LoS）データセットに対する周期3出力1パターンおよび周期2出力1パターンの補間精度評価を表\ref{tab:marunouchi_ssp_sp_results}に示す．

\begin{table}[H]
  \centering
  \caption{八重洲（LoS）データセットにおける周期3出力1および周期2出力1パターンの補間精度}
  ...
  \begin{tabular}{llrr}
    ...
    多項式近似 & 2次，片側3点ずつ & 0.0969 & 0.118 \\
    多項式近似 & 3次，片側3点ずつ & 0.0969 & 0.118 \\
    多項式近似 & 4次，片側3点ずつ & \textbf{0.0278} & \textbf{0.0370} \\
    多項式近似 & 5次，片側3点ずつ & \textbf{0.0278} & \textbf{0.0370} \\
    多項式近似 & 6次，片側3点ずつ & 0.164 & 0.224 \\
    多項式近似 & 4次，片側6点ずつ & 0.107 & 0.132 \\
    多項式近似 & 4次，片側9点ずつ & 0.133 & 0.143 \\
    ニューラルネットワーク補間 & $c=1$ & 0.140 & 0.142 \\
    ニューラルネットワーク補間 & $c=2$ & 0.197 & 0.166 \\
    ニューラルネットワーク補間 & $c=3$ & 0.174 & 0.227 \\
    \hline
  \end{tabular}
\end{table}
...
\noindent
八重洲（LoS）データセットに対する周期3・2出力パターンおよび周期4・3出力パターンの補間精度評価をそれぞれ表\ref{tab:marunouchi_los_spp_results}および表\ref{tab:marunouchi_los_sppp_results}に示す．
本実験では補間位置が複数存在するため，補間位置ごとのNMSE（pos）と，それらを統合したOverall NMSEを算出した．

\begin{table}[H]
  \centering
  \caption{八重洲（LoS）データセットにおける周期3・2出力パターンの補間精度}
  \label{tab:marunouchi_los_spp_results}
  \begin{tabular}{llrrr}
    \hline
    手法 & 設定 & Overall NMSE & pos0 & pos1 \\
    \hline
    多項式近似 & 5次，片側3点ずつ & \textbf{0.168} & 0.165 & 0.171 \\
    スプライン補間 & 片側3点ずつ & 0.174 & 0.171 & 0.177 \\
    スプライン補間 & 片側6点ずつ & 0.176 & 0.176 & 0.176 \\
    スプライン補間 & 片側9点ずつ & 0.176 & 0.176 & 0.176 \\
    多項式近似 & 3次，片側3点ずつ & 0.179 & 0.179 & 0.179 \\
    多項式近似 & 6次，片側6点ずつ & 0.183 & 0.183 & 0.183 \\
    多項式近似 & 2次，片側3点ずつ & 0.185 & 0.181 & 0.188 \\
    多項式近似 & 4次，片側3点ずつ & 0.199 & 0.191 & 0.208 \\
    多項式近似 & 5次，片側6点ずつ & 0.225 & 0.225 & 0.225 \\
    多項式近似 & 4次，片側6点ずつ & 0.226 & 0.226 & 0.226 \\
    多項式近似 & 6次，片側9点ずつ & 0.226 & 0.226 & 0.226 \\
    多項式近似 & 4次，片側9点ずつ & 0.257 & 0.257 & 0.257 \\
    多項式近似 & 5次，片側9点ずつ & 0.257 & 0.257 & 0.257 \\
    多項式近似 & 6次，片側3点ずつ & 0.452 & 0.452 & 0.452 \\
    \hline
  \end{tabular}
\end{table}
...
\begin{table}[H]
  \centering
  \caption{八重洲（LoS）データセットにおける周期4・3出力パターンの補間精度}
  \label{tab:marunouchi_los_sppp_results}
  \begin{tabular}{llrrrr}
    \hline
    手法 & 設定 & Overall NMSE & pos0 & pos1 & pos2 \\
    \hline
    多項式近似 & 5次，片側3点ずつ & \textbf{0.257} & 0.204 & 0.360 & 0.206 \\
    スプライン補間 & 片側6点ずつ & 0.262 & 0.209 & 0.370 & 0.206 \\
    スプライン補間 & 片側9点ずつ & 0.262 & 0.209 & 0.370 & 0.206 \\
    スプライン補間 & 片側3点ずつ & 0.262 & 0.208 & 0.368 & 0.209 \\
    多項式近似 & 3次，片側3点ずつ & 0.272 & 0.244 & 0.324 & 0.247 \\
    多項式近似 & 5次，片側6点ずつ & 0.276 & 0.261 & 0.307 & 0.260 \\
    多項式近似 & 4次，片側6点ずつ & 0.281 & 0.269 & 0.307 & 0.268 \\
    多項式近似 & 6次，片側9点ずつ & 0.287 & 0.275 & 0.310 & 0.275 \\
    多項式近似 & 2次，片側3点ずつ & 0.287 & 0.265 & 0.324 & 0.273 \\
    多項式近似 & 6次，片側6点ずつ & 0.288 & 0.265 & 0.335 & 0.264 \\
    多項式近似 & 4次，片側3点ずつ & 0.291 & 0.247 & 0.360 & 0.265 \\
    多項式近似 & 5次，片側9点ずつ & 0.295 & 0.289 & 0.306 & 0.288 \\
    多項式近似 & 4次，片側9点ずつ & 0.298 & 0.293 & 0.306 & 0.293 \\
    多項式近似 & 6次，片側3点ずつ & 0.543 & 0.433 & 0.770 & 0.426 \\
    \hline
  \end{tabular}
\end{table}

八重洲（LoS）データセットにおいても同様の傾向が確認された．
スプライン補間と4次多項式近似は周期3出力1パターンでNMSE 0.03程度，周期2出力1パターンでNMSE 0.04程度を達成した．
...
八重洲（NLoS）データセットに対する周期3出力1パターンおよび周期2出力1パターンの補間精度評価を表\ref{tab:marunouchi_nlos_ssp_sp_results}に示す．

\begin{table}[H]
  \centering
  \caption{八重洲（NLoS）データセットにおける周期3出力1および周期2出力1パターンの補間精度}
  \label{tab:marunouchi_nlos_ssp_sp_results}
  \begin{tabular}{llrr}
    \hline
    手法 & 設定 & 周期3出力1 & 周期2出力1 \\
    \hline
    スプライン補間 & 片側3点ずつ & 0.0550 & \textbf{0.0532} \\
    多項式近似 & 2次，片側3点ずつ & 0.134 & 0.170 \\
    多項式近似 & 3次，片側3点ずつ & 0.134 & 0.170 \\
    多項式近似 & 4次，片側3点ずつ & \textbf{0.0545} & 0.0550 \\
    多項式近似 & 5次，片側3点ずつ & \textbf{0.0545} & 0.0550 \\
    多項式近似 & 6次，片側3点ずつ & 0.189 & 0.250 \\
    多項式近似 & 4次，片側6点ずつ & 0.160 & 0.194 \\
    多項式近似 & 4次，片側9点ずつ & 0.218 & 0.280 \\
    \hline
  \end{tabular}
\end{table}

\noindent
八重洲（NLoS）データセットに対する周期3・2出力パターンおよび周期4・3出力パターンの補間精度評価をそれぞれ表\ref{tab:marunouchi_nlos_spp_results}および表\ref{tab:marunouchi_nlos_sppp_results}に示す．
本実験では補間位置が複数存在するため，補間位置ごとのNMSE（pos）と，それらを統合したOverall NMSEを算出した．

\begin{table}[H]
  \centering
  \caption{八重洲（NLoS）データセットにおける周期3・2出力パターンの補間精度}
  \label{tab:marunouchi_nlos_spp_results}
  \begin{tabular}{llrrr}
    \hline
    手法 & 設定 & Overall NMSE & pos0 & pos1 \\
    \hline
    多項式近似 & 2次，片側3点ずつ & 0.212 & 0.213 & 0.212 \\
    多項式近似 & 3次，片側3点ずつ & 0.209 & 0.209 & 0.209 \\
    多項式近似 & 4次，片側3点ずつ & 0.182 & 0.182 & 0.182 \\
    多項式近似 & 5次，片側3点ずつ & \textbf{0.153} & 0.153 & 0.153 \\
    多項式近似 & 6次，片側3点ずつ & 0.458 & 0.458 & 0.458 \\
    多項式近似 & 4次，片側6点ずつ & 0.281 & 0.281 & 0.281 \\
    多項式近似 & 5次，片側6点ずつ & 0.279 & 0.279 & 0.279 \\
    多項式近似 & 6次，片側6点ずつ & 0.210 & 0.211 & 0.210 \\
    多項式近似 & 4次，片側9点ずつ & 0.346 & 0.346 & 0.346 \\
    多項式近似 & 5次，片側9点ずつ & 0.346 & 0.346 & 0.346 \\
    多項式近似 & 6次，片側9点ずつ & 0.309 & 0.309 & 0.309 \\
    スプライン補間 & 片側3点ずつ & 0.157 & 0.157 & 0.157 \\
    スプライン補間 & 片側6点ずつ & 0.161 & 0.161 & 0.161 \\
    スプライン補間 & 片側9点ずつ & 0.161 & 0.161 & 0.161 \\
    \hline
  \end{tabular}
\end{table}
...
\begin{table}[H]
  \centering
  \caption{八重洲（NLoS）データセットにおける周期4・3出力パターンの補間精度}
  ...
  \begin{tabular}{llrrrr}
    ...
    手法 & 設定 & Overall NMSE & pos0 & pos1 & pos2 \\
    \hline
    多項式近似 & 2次，片側3点ずつ & 0.317 & 0.306 & 0.340 & 0.306 \\
    多項式近似 & 3次，片側3点ずつ & 0.298 & 0.277 & 0.340 & 0.277 \\
    多項式近似 & 4次，片側3点ずつ & 0.301 & 0.268 & 0.368 & 0.267 \\
    多項式近似 & 5次，片側3点ずつ & \textbf{0.262} & 0.209 & 0.368 & 0.209 \\
    多項式近似 & 6次，片側3点ずつ & 0.548 & 0.437 & 0.781 & 0.427 \\
    多項式近似 & 4次，片側6点ずつ & 0.362 & 0.354 & 0.378 & 0.354 \\
    多項式近似 & 5次，片側6点ずつ & 0.358 & 0.348 & 0.378 & 0.348 \\
    多項式近似 & 6次，片側6点ずつ & 0.325 & 0.310 & 0.355 & 0.310 \\
    多項式近似 & 4次，片側9点ずつ & 0.376 & 0.373 & 0.382 & 0.373 \\
    多項式近似 & 5次，片側9点ずつ & 0.374 & 0.370 & 0.382 & 0.370 \\
    多項式近似 & 6次，片側9点ずつ & 0.361 & 0.354 & 0.376 & 0.354 \\
    スプライン補間 & 片側3点ずつ & 0.269 & 0.214 & 0.379 & 0.213 \\
    スプライン補間 & 片側6点ずつ & 0.271 & 0.216 & 0.382 & 0.215 \\
    スプライン補間 & 片側9点ずつ & 0.271 & 0.216 & 0.382 & 0.215 \\
    \hline
  \end{tabular}%
  }
\end{table}

周期4出力1，周期3出力1，周期2出力1パターンを通じた結果を総括する．
表\ref{tab:summary_all_patterns}に，各疎化パターンにおける補間手法の平均NMSEを示す．

\begin{table}[H]
  \centering
  \caption{周期4出力1，周期3出力1，周期2出力1パターンにおける平均NMSE}
  \label{tab:summary_all_patterns}
  \begin{tabular}{llrrr}
    \hline
    手法 & 設定 & 錦糸町 & 八重洲（LoS） & 八重洲（NLoS） \\
    \hline
    スプライン補間 & -- & 0.00840 & 0.0365 & 0.0552 \\
    多項式近似 & 4次，片側3点ずつ & 0.00970 & 0.0370 & 0.0558 \\
    多項式近似 & 5次，片側3点ずつ & 0.00970 & 0.0370 & 0.0558 \\
    \hline
  \end{tabular}
\end{table}

スプライン補間と4次多項式近似は，パイロット疎化率が25\%から50\%に変化しても安定した補間精度を維持した．
錦糸町では両手法の平均NMSEは0.01以下であり，八重洲（LoS）では0.04以下，八重洲（NLoS）においても0.06未満を維持した．
本評価の結果から，チャネル予測モデルの重み更新に用いる補間手法として，スプライン補間または4次多項式近似が有効であることが示唆された．


\section{おわりに}
本章では，チャネル予測モデルの重み更新に用いる補間手法を選定するため，錦糸町駅前と八重洲の2種類の地形データから生成したCSI系列に対して補間精度の比較実験を実施した．
比較対象として，スプライン補間および多項式近似を用いた．

実験の結果，スプライン補間と4次多項式近似が両データセットで最良の精度を達成し，錦糸町ではNMSEは0.01以下，八重洲では0.04から0.06程度であった．
